class SortingRobot:
    ...
    def sort(self):
        ...
        """
        # Fill this out
        # a list given to the robot is in front of it, and we can only compare 1 item to another at a time
        # sorting from left to right
        # bubble sort seems best, grab an item compare it to next item. if bigger, then swap, if not, continue right

        # need a way to stop sorting, will use light
        self.set_light_on()  # will turn light on
        while self.light_is_on() == True:  # so long as light is on, will continue sorting
            # now to move to the right comparing items
            # turn light off now that loop has started, if no sorting is done then loop ends
            self.set_light_off()
